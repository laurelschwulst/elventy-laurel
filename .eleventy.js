...
module.exports = function (eleventyConfig) {
  ...
  eleventyConfig.addCollection("allCollaborators", function (collection) {
    ...
    collection.getAll().forEach(function (item) {
      if (
        "collaborator" in item.data &&
        Array.isArray(item.data.collaborator)
      ) {
  ...
  eleventyConfig.addCollection("selected_section", function (collectionApi) {
    return collectionApi
      .getAll()
      .filter((item) => item.data.selected_section === true);
  });

  eleventyConfig.addCollection(
    "selected_section_shuffled",
    function (collectionApi) {
      const selected = collectionApi
        .getAll()
        .filter((item) => item.data.selected_section === true);
